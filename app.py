...
def run():
    # Акселератор
    schedule.every(1).monday.at("09:00").do(sch_msg.RnP)
    schedule.every(1).sunday.at("04:37").do(sch_msg.Test)

    # Рассылка Telegram
    schedule.every(1).friday.at("13:35").do(sch_msg.chats_message_1)
    ...
    schedule.every(1).friday.at("13:30").do(sch_msg.chats_message_7)
